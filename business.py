...
# Load data from CSV
data = pd.read_csv('ishak.csv')  # Make sure 'products.csv' is the correct file path
...
if search_option == "Product Name":
    ...
    product_name = st.selectbox("Select a Product Name", [""] + list(data['name'].unique()))

    # Filter and display the data for the selected product
    filtered_data = data[data['name'] == product_name]
...
# Search by Product Name
    ...
    if product_search:
        ...
        filtered_products = data[data['name'].str.contains(product_search, case=False, na=False)]
    else:
    ...
    if not filtered_products.empty:
        ...
        st.write(filtered_products[['name', 'dp_price', 'mrp_price', 'group_name', 'available_stock']])
    else:
...
# Search by Group Name
    ...
    group_name = st.selectbox("Select a Group", ["Premium", "Prestige", "Popular", "Prominent","Pacific","Prado","Winner","Bikrampur Plastic","Taj Plastic",
                                                 "Lira Plastic","KRM Plastic","Bengal Plastic","Akij Plastic","Tel","Diamond Plastic","Dhaka Steel",
                                                 "Dhaka Plastic", "Jaime Plastic", "Al Madina Plastic","Bengal Melamine","RFL Melamine","Plastic Jar","Dhaka Bottle","Matador","Partex"])

    # Filter and display the data for the selected group
    filtered_data = data[data['group_name'] == group_name]
    st.write(filtered_data[['name', 'dp_price', 'mrp_price', 'group_name', 'available_stock']])
